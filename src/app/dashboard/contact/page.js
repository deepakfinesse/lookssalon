...
const TableRow = memo(function TableRow({ msg, onStatusChange, onView, updatingId }) {
  const busy = updatingId === msg._id;
  ...
  return (
    <tr className="border-b border-black/5 transition-colors duration-150 hover:bg-primary/5">
      <td className="px-4 py-4 align-middle">
        <span className="text-primary text-xs font-mono font-bold">{msg.inquiryId}</span>
      </td>
      <td className="px-4 py-4 align-middle">
        <p className="text-foreground text-sm m-0 leading-snug font-medium">{msg.name}</p>
        <p className="text-grey/60 text-xs m-0 mt-0.5">{msg.email}</p>
      </td>
      <td className="px-4 py-4 align-middle text-grey text-sm">{msg.phone}</td>
      <td className="px-4 py-4 align-middle text-grey text-sm max-w-[200px] truncate">{msg.subject}</td>
      <td className="px-4 py-4 align-middle">
        <select
          value={msg.status}
          disabled={busy}
          onChange={e => onStatusChange(msg._id, e.target.value)}
...
export default function ContactDashboardPage() {
  ...
  const [stats,        setStats]       = useState({ total:0, new:0, reviewed:0, replied:0, closed:0 });
  ...
  const updateMessage = useCallback(async (id, updates) => {
    ...
    try {
      ...
      setMessages(prev => prev.map(m => m._id === id ? { ...m, ...srv } : m));
  ...
  const handleStatusChange = useCallback((id, status) => updateMessage(id, { status }), [updateMessage]);
  const handleSaveNotes    = useCallback(() => selected && updateMessage(selected._id, { notes }), [selected, notes, updateMessage]);
  ...
  const downloadCSV = useCallback(async () => {
    ...
    try {
      ...
      const COLS = ["Inquiry ID","Name","Phone","Email","Subject","Query","Status","Submitted At"];
      const escape = (v) => `"${String(v ?? "").replace(/"/g, '""')}"`;
      const lines  = [
        COLS.join(","),
        ...rows.map(m => [
          m.inquiryId, m.name, m.phone, m.email,
          m.subject, m.query, m.status, fmt(m.createdAt),
        ].map(escape).join(",")),
      ];
  ...
  const openModal  = useCallback((msg) => { setSelected(msg); setNotes(msg.notes || ""); }, []);
    ...
    <>
      ...
      <main className="px-8 py-8 max-w-[1400px] mx-auto">
        ...
        <div className="flex gap-3 mb-8 flex-wrap">
          ...
          <StatCard label="Replied"  value={stats.replied}  valueCls="text-emerald-600" />
        ...
        <div className="flex gap-3 mb-6 flex-wrap items-center">
          <input
            type="search"
            placeholder="Search by name, email, phone, subject, ID…"
            value={search}
        ...
        <div className="bg-white border border-black/10 rounded overflow-hidden shadow-sm">
          {loading ? (
            ...
            <div className="py-16 text-center text-grey/60">
              ...
              <p className="text-base">No messages found.</p>
            </div>
          ) : (
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-black/[0.02]">
                    {["Message ID","Name","Phone","Subject","Status","Date",""].map((h, i) => (
                      <th key={i}
          ...
          <div className="bg-white border border-black/10 rounded w-full max-w-[580px] max-h-[90vh] overflow-y-auto shadow-xl">
            ...
            <div className="p-7">
              ...
              <div className="grid grid-cols-2 gap-5 mb-6">
                {[
                  ["Message ID", <span key="id" className="text-primary font-mono text-sm font-bold">{selected.inquiryId}</span>],
              ...
              <div className="mb-5">
                <p className="text-grey/60 text-xs tracking-[2px] uppercase mb-2 font-bold">Subject</p>
                <p className="text-foreground text-sm font-medium m-0">{selected.subject}</p>
              </div>

              <div className="mb-5">
                <p className="text-grey/60 text-xs tracking-[2px] uppercase mb-2 font-bold">Query</p>
                <p className="text-foreground text-sm bg-black/[0.02] border border-black/10 rounded p-3 m-0 whitespace-pre-wrap">
                  {selected.query}
                </p>
              </div>
                  ...
                  {STATUS_KEYS.map(val => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => updateMessage(selected._id, { status: val })}
