...
const STATUS_CFG = {
  pending: {
    ...
    activeBtnCls:"text-amber-400 bg-amber-400/10 border border-amber-400/30 font-bold",
  },
  confirmed: {
    label:       "Confirmed",
    badgeCls:    "text-emerald-400 bg-emerald-400/10 border border-emerald-400/30",
    selectCls:   "text-emerald-400",
    activeBtnCls:"text-emerald-400 bg-emerald-400/10 border border-emerald-400/30 font-bold",
  },
  completed: {
    label:       "Completed",
    badgeCls:    "text-indigo-400 bg-indigo-400/10 border border-indigo-400/30",
    selectCls:   "text-indigo-400",
    activeBtnCls:"text-indigo-400 bg-indigo-400/10 border border-indigo-400/30 font-bold",
  },
  cancelled: {
    label:       "Cancelled",
    badgeCls:    "text-red-400 bg-red-400/10 border border-red-400/30",
    selectCls:   "text-red-400",
    activeBtnCls:"text-red-400 bg-red-400/10 border border-red-400/30 font-bold",
  },
};
...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`inline-block px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wide uppercase ${cfg.badgeCls}`}>
      {cfg.label}
    </span>
  );
}
...
function StatCard({ label, value, valueCls = "text-white" }) {
  return (
    <div className="flex-1 min-w-[120px] bg-[#111] border border-[#222] rounded p-5">
      <p className="text-[#555] text-[10px] tracking-[2px] uppercase mb-2.5 m-0">{label}</p>
      <p className={`font-[Georgia,serif] text-[28px] font-light m-0 ${valueCls}`}>{value}</p>
    </div>
  );
}
  ...
  return (
    <tr className="border-b border-[#161616] transition-colors duration-150 hover:bg-[rgba(201,168,76,0.04)]">
      ...
      <td className="px-4 py-3.5 align-middle">
        <span className="text-[#c9a84c] text-[11px] font-mono">#{appt.bookingId}</span>
      </td>
      ...
      <td className="px-4 py-3.5 align-middle text-[#888] text-xs">{appt.contact}</td>
      <td className="px-4 py-3.5 align-middle text-[#888] text-xs max-w-[160px]">{appt.service}</td>
      <td className="px-4 py-3.5 align-middle text-[#888] text-xs">{appt.city}</td>
      <td className="px-4 py-3.5 align-middle text-[#888] text-xs">{appt.preferredTime}</td>
      ...
      */}
      ...
      >
        <div className="flex items-center gap-3">
          <span className="text-[#c9a84c] text-[11px] tracking-[4px] uppercase">LOOKS SALON</span>
      ...
      <main className="px-8 py-8 max-w-[1400px] mx-auto">
        ...
        <div className="flex gap-3 mb-8 flex-wrap">
          ...
          <StatCard label="Pending"   value={stats.pending}   valueCls="text-amber-400"   />
          <StatCard label="Confirmed" value={stats.confirmed} valueCls="text-emerald-400" />
          <StatCard label="Completed" value={stats.completed} valueCls="text-indigo-400"  />
          <StatCard label="Cancelled" value={stats.cancelled} valueCls="text-red-400"     />
        </div>
          ...
          <div className="bg-[#111] border border-[#222] rounded w-full max-w-[560px] max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-7">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                {[
                  ["Booking ID", <span key="id" className="text-[#c9a84c] font-mono text-xs">#{selected.bookingId}</span>],
                  ...
                  <div key={label}>
                    <p className="text-[#444] text-[10px] tracking-[2px] uppercase m-0 mb-1.5">{label}</p>
              ...
              <div className="mb-5">
                <p className="text-[#444] text-[10px] tracking-[2px] uppercase mb-2">Current Status</p>
                <StatusBadge status={selected.status} />
              </div>

              {/* Status update buttons */}
              <div className="mb-5">
                <p className="text-[#444] text-[10px] tracking-[2px] uppercase mb-2.5">Update Status</p>
                <div className="flex gap-2 flex-wrap">
                  {STATUS_KEYS.map(val => {
