...
const Nav = ({ isActive, toggleMenu }) => {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 bg-black/90 flex flex-col items-center gap-6 p-4 transition-transform duration-700 ${
        ...
        {menuItems.map((item) => (
          <li key={item.label} className="mb-2 lg:mb-2 xl:mb-2">
            <Link
              ...
              className="text-xl sm:text-3xl md:text-3xl lg:text-3xl xl:text-[48px] font-extrabold bg-gradient-to-b from-primary to-[#DFAB31] bg-[length:100%_220%] bg-bottom bg-clip-text text-transparent transition-all duration-500 hover:bg-top hover:scale-110 flex items-center uppercase gap-2"
            >
              {item.label}
              {item.subItems &&
                (showSubServices ? (
